...
\subsection{Implemented Risk Metric Layer Outputs}

Risk Metric Layer v1 translates the scenario-specific shock outputs into the paper's formal household fuel price risk indicators. The layer uses S2, S3, and S6 Q3 as the main risk scenario set, retains S0 as the baseline reference, uses S4 as the policy-mitigation comparison, and treats S1 as a sensitivity case. The central payment-method track remains Direct Debit. The property-level extract contains 124,477 valid repricing rows, and the QA summary reports 14 passed checks out of 14.

The main manuscript indicator is \texttt{FPaR\_P90}, the 90th-percentile Fuel Price-at-Risk burden across the main risk scenario set. At property level, the mean \texttt{FPaR\_P90} is 7.45 percent of required income, the median is 6.34 percent, and the 90th percentile of property-level \texttt{FPaR\_P90} is 12.39 percent. The corresponding mean threshold-crossing risk above a 10 percent required-energy-burden threshold is 10.34 percent. The P90 affordability gap-at-risk is approximately \pounds 1,202 per year at property level. These are required-energy risk metrics rather than observed bills or official fuel-poverty rates.

Figure~\ref{fig:risk-metric-fpar-spatial} maps mean \texttt{FPaR\_P90} across Westminster LSOAs. It shows that upper-tail fuel price risk is spatially uneven even within a high-income borough. The highest mean LSOA \texttt{FPaR\_P90} values include E01004646 at 12.98 percent, E01004720 at 11.78 percent, E01033605 at 11.25 percent, and E01004719 at 11.15 percent. The map matches 123 of 126 result LSOAs to the current GIS file; the unmatched result LSOAs are retained in tabular summaries but omitted from the mapped panel.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{paper/figures/risk_metric_fpar_p90_spatial_distribution.png}
\caption{Spatial distribution of mean household Fuel Price-at-Risk P90 across Westminster LSOAs, expressed as required energy burden under the main risk scenario set. The map matches 123 of 126 result LSOAs to the current Westminster GIS boundary file.}
\label{fig:risk-metric-fpar-spatial}
\end{figure}

Figure~\ref{fig:risk-metric-archetypes} identifies the highest-risk household-dwelling archetypes by mean \texttt{FPaR\_P90}. The top groups are dominated by gas-heated houses and maisonettes with weaker EPC performance. Owner-occupied houses in EPC E--G with gas heating have a mean \texttt{FPaR\_P90} of 19.3 percent, threshold risk of 71.1 percent, and \texttt{AGaR\_P90} of approximately \pounds 4,340 per year. Private rented houses in EPC E--G with gas heating follow at 16.5 percent, 57.8 percent, and approximately \pounds 3,112 per year. The figure therefore sharpens the paper's distributional claim: the highest fuel price risk is produced by tenure, dwelling form, fabric, fuel, and income sensitivity together.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{paper/figures/risk_metric_top_archetype_risk_metrics.png}
\caption{Top Westminster household-dwelling archetypes ranked by Fuel Price-at-Risk P90, with corresponding threshold-crossing risk and affordability gap-at-risk.}
\label{fig:risk-metric-archetypes}
\end{figure}

Figure~\ref{fig:risk-metric-policy-residual} reports residual S4 burden relative to S3 for the same high-risk archetypes. The mean S4/S3 burden residual in the property-level layer is 74.11 percent, indicating that policy mitigation reduces the severity of shock exposure but does not eliminate the structural concentration of risk. This is why S4 should be interpreted as partial policy buffering, not as household de-risking in the stronger sense used for intervention counterfactuals.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{paper/figures/risk_metric_policy_mitigation_residual.png}
\caption{Residual S4 burden relative to S3 among high-risk archetypes, showing that policy mitigation reduces but does not eliminate structurally concentrated household fuel price risk.}
\label{fig:risk-metric-policy-residual}
\end{figure}
...
Expected tables will summarise baseline burden probabilities, modelled fuel-poverty diagnostics, price-shock burden uplift, Fuel Price-at-Risk, and intervention-specific de-risking dividends by income group, EPC rating, dwelling form, heating system, tenure, payment method, and LSOA/MSOA. The baseline extract figures now promoted into the manuscript are Figures~\ref{fig:baseline-burden-distribution}--\ref{fig:baseline-high-burden-main-fuel}; the current high-risk explanation figures are Figures~\ref{fig:hr-br-overlap}--\ref{fig:spatial-high-risk}; the official price-spine figures are Figures~\ref{fig:official-price-spine-timeline}--\ref{fig:official-price-spine-s6}; the first price-shock result figures are Figures~\ref{fig:shock-mechanism-heatmaps}--\ref{fig:shock-spatial-distribution}; and the formal risk-metric figures are Figures~\ref{fig:risk-metric-fpar-spatial}--\ref{fig:risk-metric-policy-residual}. Remaining expected figures will include: the conceptual distinction between fuel poverty, energy burden, and household fuel price risk; intervention de-risking comparisons; and maps of de-risking gaps.
